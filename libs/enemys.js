...
enemys.prototype.getDamageString = function (enemy, x, y, floorId) {
    ...
    if (damage == null) {
        ...
        if (core.enemys.hasSpecial(enemy, 11))
            damage += "^";
        if (core.enemys.hasSpecial(enemy, 53)) 
            damage += "蓝";
        if (core.enemys.hasSpecial(enemy, 54)) 
            damage += "红";
        if (core.enemys.hasSpecial(enemy, 55)) 
            damage += "绿";
        if (core.enemys.hasSpecial(enemy, 56)) 
            damage += "黄";
        if (core.enemys.hasSpecial(enemy, 57)) 
            damage += "开";
        if (core.enemys.hasSpecial(enemy, 101)) 
            damage += "爆";
        if (core.enemys.hasSpecial(enemy, 102)) 
            damage += "白";
        if (core.enemys.hasSpecial(enemy, 103)) 
            damage += "黑";
        if (core.enemys.hasSpecial(enemy, 108)) 
            damage += "毁";
    }
